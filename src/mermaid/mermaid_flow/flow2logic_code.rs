...
/// Check if the node name corresponds to a function name, and return the function name if it does. The mapping from node name to function name is defined in the is_clang_function.ini file. If the node name does not correspond to any function name, return an error.
fn is_node_name_function_name(node_name:&str) -> Result<String, String> {
    let is_function_mapping = ini2hash!("src/mermaid/mermaid_flow/is_clang_function.ini");
